(function ($) {
	...
	$(document).ready(function () {
		lottie.loadAnimation({
			container: $('#lottie-container')[0], // the dom element that will contain the animation
			renderer: 'svg', // the render type: 'svg', 'canvas', or 'html'
			loop: true, // whether to loop the animation
			autoplay: true, // whether to start playing the animation
			path: '../assets/js/JMZYYnwJvP.json' // the path to the animation json
		});
	});
	// Common-slider-over
	...
	if (filter_price.length) {
	 ...
	 function updateActiveLink() {
		 ...
		 if ($(window).scrollTop() === 0) {
			 currentComponent = 'rtsHeader'; // or whatever ID you use for the Home component
		 } else {
